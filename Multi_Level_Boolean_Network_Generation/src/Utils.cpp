...
int findDefinition(map<int, string>& definition, string name){
    for(auto &it : definition)
        if(it.second==name) return it.first ;
    return -1 ;
}
void Cubes::recoverVariable(string recoveredVariable){
    if(decodeTable.find(recoveredVariable)==decodeTable.end()) throw invalid_argument( "Boolean錯誤符號 : " + recoveredVariable + "\n") ;
    
    int replcedPosition = decodeTable[recoveredVariable] ;
    BooleanPOS booleanDefnition = at(findDefinition(positionalTable, recoveredVariable)) ;
    
    for(int i=0; i<size(); i++){
        BooleanPOS tem1, tem2 ;
        for(int j=0; j<at(i).size(); j++){
            if(at(i)[j][replcedPosition]==1){
                at(i)[j][replcedPosition] = 0 ;
                tem1.push_back(at(i)[j]) ;
            }else{
                tem2.push_back(at(i)[j]) ;
            }
        }
        tem1 = tem1 * booleanDefnition ;
        for(auto &it : tem1) tem2.push_back(it) ;
        at(i) = tem2 ;
    }
}

void Cubes::multipleLevelToTwoLevel(void){
    set<string> replacedVariables ;
    for(auto &it : decodeTable){
        if(it.first.back()!='\'')
            if(find(inputTable.begin(), inputTable.end(), it.first) == inputTable.end() &&
                find(outputTable.begin(), outputTable.end(), it.first) == outputTable.end())
                replacedVariables.insert(it.first) ;
    }
    //for(auto &it : replacedVariables) cout << it <<" " ;

    for(auto &it : replacedVariables) recoverVariable(it) ;

    Cubes twoLevel ;
    map<int, string> newPositionalTable ;

    for(int i=0; i<size(); i++){
        if(positionalTable.find(i)!=positionalTable.end() &&
           find(replacedVariables.begin(), replacedVariables.end(), positionalTable[i]) == replacedVariables.end()){
            newPositionalTable[twoLevel.size()] =  positionalTable[i] ;
            twoLevel.push_back(at(i)) ;
        }
    }
    positionalTable = newPositionalTable ;
    clear() ;
    for(int i=0; i<twoLevel.size(); i++) push_back(twoLevel[i]) ;
