    ...
    public :
        ...
        void Parse(int argc, char* argv[]){
            ...
            for(int i=1; i<argc; i++){
                ...
                else if(arg=="-r") recoverTwoLevel = true ;
                else if(!blifFile.empty()) outBlifFile = arg ;
                else if(arg.substr(max((int)arg.size()-5,0),arg.size())==".blif") blifFile = arg ;
            }
            if(blifFile.empty()) throw invalid_argument( "未指定輸入BLIF檔案\n") ;
            if(outBlifFile.empty()) throw invalid_argument( "未指定輸出BLIF檔案\n") ;
        }
...
int main(int argc, char* argv[]){
    ...
    cout << "----------------------------------------------\n" ;
    cout << "Input File Name: " << ExecuteConfig.blifFile <<"\n" ;
    cout << "Output File Name: " << ExecuteConfig.outBlifFile <<"\n" ;
    ...
    if(ExecuteConfig.showBoolean){
        cout << optCubes << "\n" ;
    }else if(ExecuteConfig.recoverTwoLevel){
        optCubes.outputTable.clear();
        for(auto &it : CubeCreator.outputVariables) optCubes.outputTable.push_back(it) ;
        optCubes.multipleLevelToTwoLevel() ;
        optCubes.writeVariables(CubeCreator.modelName, "recover.blif") ;
    }else{
        ...
        optCubes.writeVariables(CubeCreator.modelName, ExecuteConfig.outBlifFile) ;
        cout << "Optimized Literal Count: " << optCubes.literalCount() << "\n";
    }
    
    cout << "Execution Time: " << float(clock() - clockVariable)/CLOCKS_PER_SEC << " Sec\n";
    cout << "----------------------------------------------\n" ;
    return 0 ;
}
